# --------------------------------------------------
# Create main class
# --------------------------------------------------
class Geometria:
    def __init__(self, valores):
        self.valores = valores

    def area(self):
        pass

    def formula(self):
        pass


# --------------------------------------------------
# Create derived classes with abstract methods
# --------------------------------------------------
class cuadrado(Geometria):
    def area(self):
        return self.valores[0] ** 2

    def formula(self):
        return 'Base * Base'

    def datos(self):
        return self.valores[0]


class rectangulo(Geometria):
    def area(self):
        return self.valores[0] * self.valores[1]

    def formula(self):
        return 'Base * Altura'

    def datos(self):
        return str(self.valores[0]) + ', ' + str(self.valores[1])


class Triangulo(Geometria):

    def area(self):
        return self.valores[0] * self.valores[1] * self.valores[2]

    def formula(self):
        return 'Lado * Lado * Lado'
    def tipo_triangulo(self):
        if str(self.valores[0]) == str(self.valores[1]) == str(self.valores[2]):
            return "El triángulo es equilátero."
        elif str(self.valores[0]) == str(self.valores[1]) or str(self.valores[0]) == str(self.valores[2]) or str(self.valores[1]) == str(self.valores[2]):
            return "El triángulo es isósceles."
        else:
            return "El triángulo es escaleno."

# --------------------------------------------------
# Using classes
# --------------------------------------------------
fig1 = cuadrado([3])
print('La formula del cuadrado es {0}.'.format(fig1.formula()))
print('Y con la siguiente lista de valores [{0}] obtenemos un área de {1}'.format(fig1.datos(), fig1.area()))

print()

fig2 = rectangulo([3, 5])
print('La formula del resctángulo es {0}.'.format(fig2.formula()))
print('Y con la siguiente lista de valores [{0}] obtenemos un área de {1}'.format(fig2.datos(), fig2.area()))

fig3 = Triangulo([4, 3, 5])
print("La formula del triangulo es {0}".format(fig3.formula()))
print("El tipo de triangulo es: {0}".format(fig3.tipo_triangulo()))
